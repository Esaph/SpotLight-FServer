...
public class GetAllConversationMessagesLast24Hours extends EsaphCommand
{
    private static final String queryGetAllConversationMessagesAktuelle =
            "SELECT * FROM PrivateReceivers " +
                    "JOIN PrivateMoments ON " +
                    "(PrivateMoments.UID=? AND PrivateMoments.PPID=PrivateReceivers.PPID AND PrivateMoments.Time >= DATE_SUB(NOW(), INTERVAL 24 HOUR)) OR " +
                    " (PrivateReceivers.UID_REC=? AND PrivateMoments.PPID = PrivateReceivers.PPID AND PrivateMoments.Time >= DATE_SUB(NOW(), INTERVAL 24 HOUR)) ORDER BY PrivateMoments.PPID";
    ...
    public GetAllConversationMessagesLast24Hours(InformationServer plServer, InformationServer.RequestHandler requestHandler, LogUtilsEsaph logUtilsRequest)
    {
        super(plServer, requestHandler, logUtilsRequest);
    }

    @Override
    public void run() throws Exception //Think about this solution. This only works if the data is correctyl sorted between.
            //Update: Fixed this problem, we sorting the data by PPID, so everything which belongs to the post, is grouped in one section.
    {
        ...
        try
        {
            prGetAktuellePostings = (PreparedStatement) super.requestHandler().getCurrentConnectionToSql()
                    .prepareStatement(GetAllConversationMessagesLast24Hours.queryGetAllConversationMessagesAktuelle);
            ...
            {
                if(jsonObjectPost == null || jsonObjectPost.getLong("PPID") != result.getLong("PPID")) //Erstellt ein neuen beitrag. Queryd dann über die empfänger von diesem.
                {
                    ...
                    PreparedStatement checkIfSaved = (PreparedStatement) //Putting saved thinks.
                            super.requestHandler()
                                    ...
                                    .prepareStatement(GetAllConversationMessagesLast24Hours.queryGetSaversFromPrivatePost);
                    ...
                    PreparedStatement prGetHashtagFromPost = (PreparedStatement) super.requestHandler().getCurrentConnectionToSql().prepareStatement(GetAllConversationMessagesLast24Hours.queryGetHashtagsFromPost);
